...
data Eval : Expr env t -> Expr env t -> Type where
  ...
  EvPrimLetL : IsValue e12 b -> index FZ (freeVars e12) = True ->
      Eval (Prim f (Let e11 e12) e2) (Let e11 (Prim f e12 (incr FZ _ e2)))
  EvPrimLetR : IsValue e22 b -> index FZ (freeVars e22) = True ->
  ...
  EvIsZeroLet : IsValue e12 b -> index FZ (freeVars e12) = True ->
  ...
  EvAppLet : IsValue e12 b -> index FZ (freeVars e12) = True ->
  ...
  EvCaseLet : IsValue e2 b -> index FZ (freeVars e2) = True ->
  ...
  EvTyAppLet : IsValue e2 b -> index FZ (freeVars e2) = True ->
  ...
  EvUnfoldLet : IsValue e2 b -> index FZ (freeVars e2) = True ->
...
progress' (Let e1 e2) with (progress' e2)
  ...
  progress' (Let e1 e2) | VarHeaded (IxZ t1 env) vh with (progress' e1)
    progress' (Let (Let e11 e12) e2) | VarHeaded (IxZ t1 env) vh | Value {b = False} (LetVal v eq) =
        Step (Let e11 (Let e12 (incr (FS FZ) _ e2))) (EvLetLet vh v eq)
    progress' (Let e1 e2) | VarHeaded (IxZ t1 env) vh | Value {b = True} v =
        Step (Let e1 (subst (incr FZ _ e1) e2 vh)) (EvLet3 vh v)
...
progress e with (progress' e)
  progress e | Value v = Left (_ ** v)
  progress e | Step e' ev = Right (e' ** ev)
  progress e | VarHeaded ix vh impossible

export partial
evaluate : {t : Ty 0} -> (e : Expr [] t) ->
    (e' ** (Iterate Eval e e', (b ** IsValue e' b)))
