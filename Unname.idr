...
unnameXs' : (ns : Vect n Name) -> (ts : Vect n (Ty tn)) ->
    (xs : NVarArgs (multiExtend FiniteMap.emptyMap ns ts) t) -> VarArgs ts t
unnameXs' ns ts [] = []
unnameXs' ns ts (Cons x xs eq) = getIndex ns ts x eq :: unnameXs' ns ts xs

mutual
  unname' : (ns : Vect n Name) -> (ts : Vect n (Ty tn)) ->
      (e : NExpr (multiExtend FiniteMap.emptyMap ns ts) t) -> Expr ts t
  ...
  unname' ns ts (TyAbs e) =
      TyAbs (unname' ns (map (tyincr FZ) ts)
  ...
  unnameAs' : (ns : Vect n Name) -> (ts : Vect n (Ty tn)) ->
      (as : NAlts (multiExtend FiniteMap.emptyMap ns ts) ctrs t) -> Alts ts ctrs t
  unnameAs' ns ts Fail = Fail
  unnameAs' ns ts (Alt xs ts' e as) =
      Alt (unname' (xs ++ ns) (ts' ++ ts) (rewrite sym (multiExtendAppendEmpty ns ts xs ts') in e))
          (unnameAs' ns ts as)

export
unname : NExpr FiniteMap.emptyMap t -> Expr [] t
unname e = unname' [] [] e
